<template>
  <v-container>
    ...
    <v-row v-for="group in voices" :key="group">
      <v-col cols="12" class="ma-0 pa-0">
        <v-card class="ma-1 pa-0">
          <v-card-title v-if="$i18n.locale == 'zhHans'">{{
            group.translation.Chinese
          }}</v-card-title>
          <v-card-title v-else-if="$i18n.locale == 'ja'">{{
            group.translation.Japanese
          }}</v-card-title>
          <v-card-title v-else-if="$i18n.locale == 'en'">{{
            group.translation.English
          }}</v-card-title>
          <v-container>
    ...
    <v-dialog v-model="helpdialog" persistent max-width="600">
      ...
      <v-btn raised color="primary" @click="helpdialog = false">{{
        $t("ui.gotit")
      }}</v-btn>
    </v-dialog>
  </v-container>
